...
def get_brand_name():
	itemwise_batch = {};
	batches= frappe.db.sql(""" SELECT brand_name, item_code FROM `tabItem` """, as_dict=1)
	print(batches)
	for batch in batches:
		if batch.item_code not in itemwise_batch:
			itemwise_batch.setdefault(batch.item_code, [])
		itemwise_batch[batch.item_code].append(batch.brand_name)
...
def get_items_list(pos_profile, company):
	...
	return frappe.db.sql("""
		select
			i.name, i.item_code, i.item_name, i.item_type, i.brand_name, i.volume,i.item_shelf, i.description, i.item_group, i.has_batch_no,
